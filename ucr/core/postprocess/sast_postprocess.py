...
class SASTPostProcess(object):
    ...
    def expand_poly_along_width(self, poly, shrink_ratio_of_width=0.3):
        ...
        left_quad_expand = self.shrink_quad_along_width(
            left_quad, left_ratio, 1.0
        )
        right_quad = np.array(
            [
                poly[point_num // 2 - 2],
        ...
        right_quad_expand = self.shrink_quad_along_width(
            right_quad, 0.0, right_ratio
        )
    ...
    def cluster_by_quads_tco(self, tcl_map, tcl_map_thresh, quads, tco_map):
        ...
        pred_tc_tile = np.tile(
            pred_tc[:, np.newaxis, :], (1, m, 1)
        )  # (n, m, 2)
    ...
    def estimate_sample_pts_num(self, quad, xy_text):
        ...
        dense_xy_center_line_diff = (
            dense_xy_center_line[1:] - dense_xy_center_line[:-1]
        )
        estimate_arc_len = np.sum(
            np.linalg.norm(dense_xy_center_line_diff, axis=1)
        )
    ...
    def detect_sast(
        ...
        """
        # restore quad
        scores, quads, xy_text = self.restore_quad(
            tcl_map, tcl_map_thresh, tvo_map
        )
