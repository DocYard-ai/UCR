...
class ResNetFPN(nn.Module):
    def __init__(self, in_channels=1, layers=50, **kwargs):
        ...
        if layers >= 50:
            for block in range(len(self.depth)):
                for i in range(self.depth[block]):
                    ...
                    self.add_module(
                        "bottleneckBlock_{}_{}".format(block, i), blocks
                    )
            ...
            for block in range(len(self.depth)):
                for i in range(self.depth[block]):
                    ...
                    self.add_module(conv_, basic_block)
        ...
        for i in [-2, -3]:
            ...
            self.add_module("F_{}_base_block_0".format(i), block1)
            self.base_block.append(block1)
            ...
            self.add_module("F_{}_base_block_1".format(i), block2)
            self.base_block.append(block2)

            block3 = nn.BatchNorm2d(out_ch_list[i])
            self.add_module("F_{}_base_block_2".format(i), block3)
            self.base_block.append(block3)

        block4 = nn.Conv2d(
            in_channels=out_ch_list[i], out_channels=512, kernel_size=1
        )
        self.add_module("F_{}_base_block_3".format(i), block4)
        self.base_block.append(block4)
